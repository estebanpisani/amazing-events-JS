...
//Funciones
function filtrarEventosPasados(lista, fechaRef){
    let eventosPasados = [];
    for (let i = 0; i < lista.length; i++) {
        let fechaEvento = lista[i].date;
        if(fechaEvento<fechaRef){
            eventosPasados.push(lista[i]);
        }
    }
    listaEventos = eventosPasados;
}
...
async function getAPIData(url){
    ...
    filtrarEventosPasados(eventosTotal, dataAPI.currentDate)
